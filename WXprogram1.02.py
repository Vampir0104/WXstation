...
class Station:
    ...
    def setStation(self):
        ...
        self.url = [
            f'https://aviationweather.gov/api/data/metar?ids={station}&format=raw&taf=true&time=valid'
        ...
        for url in self.url:
            try:
                ...
                if not self.metar_data.strip():
                    self.report.append(f"Failed to retrieve data for: {url[47:51]}")
                    self.report.append("")
                else:
                    ...
                    if len(lines) > 0:
                        ...
                        if self.taf == "Yes" and len(lines) > 1:
                            self.report.append("TAF Data:")
                            self.report.extend(lines[1:])
                        elif self.taf == "Yes":
                            self.report.append("TAF Data:")
                            self.report.append("TAF Data not available for this aerodrome.")
                    else:
